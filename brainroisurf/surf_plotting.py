...
def plot_surf(surf_mesh, surf_map=None, bg_map=None,
              hemi=None, view='lateral', cmap=None,
    ...
    avg_method: {'mean', 'median','max'}, default is 'mean'
    ...
    if hemi == 'right':
        ...
        raise ValueError('hemi must be one of right or left or None, in case you specify elevation and azimut in the view')
    ...
    if bg_map is not None or surf_map is not None:
        ...
        elif bg_on_data is True:
            bg_data = bg_map
            #print(bg_data.shape,coords.shape)
        ...
        if surf_map is not None:
            ...
            if bg_on_data:
                ...
                # else use the colored triangles with no shading
                face_colors[kept_indices] = cmap(surf_map_faces[kept_indices])
        ...
        if kwargs.get('colorbar',False) is True:
            ...
            mappable.set_array(surf_map_data)
            
            num_modules = kwargs.get('num_modules', None)
            if num_modules is not None:
                cbar = figure.colorbar(mappable, shrink=0.75, aspect=5,
                                       boundaries = range(0, num_modules + 2),
                                       values = range(0, num_modules + 2))
                cbar.set_ticks( np.array(range(0, num_modules + 1)) + 0.5 )
                cbar.set_ticklabels( ['0'] + list(range(1, num_modules + 1)) ) # altezza dei tick labels OK
            else: # use automatic colorbar with continuos colors
                cbar = figure.colorbar(mappable, shrink = 0.75, aspect = 5)
    ...
    if output_file is not None:
        figure.savefig(output_file, dpi=72, bbox_inches='tight', pad_inches=0)
        plt.close(figure)
    else:
        return figure
...
def plot_surf_stat_map(surf_mesh, stat_map, bg_map=None,
    ...
    reflectx: bool, optional:
        Whether to reflect the x axis.
...
def plot_surf_roi(surf_mesh, roi_map, bg_map=None,
    ...
    display = plot_surf(surf_mesh, surf_map=roi_map, bg_map=bg_map,
                        hemi=hemi, view=view,  # removed avg_method keyword
